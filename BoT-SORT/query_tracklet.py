#!/usr/bin/env python3
"""
Query 圖片 → 重跑 BoT-SORT → 以人員 CLIP-ReID cosine similarity 比對 tracklet → 輸出拼圖。

僅載入 mapping JSON 中 label == "person" 的 crop（與 query_vehicle_tracklet.py 只載 car 對稱）。

給一張 query 圖片，對一支或多支鏡頭重跑 BoT-SORT 追蹤，
找出每條 tracklet 中和 query 相似度高於門檻的結果，輸出每支鏡頭的拼圖。

單支鏡頭：
  python3 query_tracklet.py \\
...
多支鏡頭：
  python3 query_tracklet.py \\
    --query-image /path/to/query.jpg \\
    --video-ids K8-09 K8-01 K8-05 K8-07 \\
    --similarity-thresh 0.80
...
DEFAULT_QUERY_IMAGE = DEFAULT_PERSON_QUERY
DEFAULT_VIDEO_IDS = ["K8-09"]
...
def person_cache_path(video_id: str) -> Path:
    return default_output_dir(video_id) / "person_clipreid_embeddings_cache.pkl"


def load_person_records(mapping_json: Path, crop_dir: Path, video_id: str) -> List[Dict[str, Any]]:
    ...
    print(f"    video_id={video_id} 共 {len(records)} 筆 person crop")
    return records
...
def process_video(
    *,
    ...
    print(f"\n========== [{video_id}] ==========")
    crop_dir = default_crop_dir(video_id)
    cache_path = person_cache_path(video_id)

    print(f"[{video_id}][1] 載入 records …")
    records = load_person_records(mapping_json, crop_dir, video_id=video_id)
    if not records:
        print(f"[{video_id}] 無 person crop 記錄，跳過")
        return
    ...
    if not matched:
        ...
        collage_path = output_dir / f"{video_id}_query_result.png"
        save_query_collage(matched=matched, out_path=collage_path)

    print(f"[{video_id}][6] 儲存 JSON …")
    json_path = output_dir / f"{video_id}_query_result.json"
    json_data = {
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Query 圖片比對人員 BoT-SORT tracklet（僅 label=person）")
    p.add_argument("--query-image", type=Path, default=DEFAULT_QUERY_IMAGE)
...
def main() -> None:
    ...
    print("[0] 初始化 ClipReIDEmbedder（人員）…")
    embedder = ClipReIDEmbedder(
