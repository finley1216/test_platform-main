...
# --- 環境變數載入（專案根 .env 優先含 MY_API_KEY；backend/.env 可覆寫）---
...
# 與本腳本同 repo 的 backend/segment，供備份 RTSP 錄製片段（ffprobe 檢查用）
_BACKEND_SEGMENT_DIR = Path(__file__).resolve().parent.parent / "segment"
...
def start_rtsp_segmenter(rtsp_url: str, duration_sec: float, chunks: int, out_dir: Path, stream_id: str):
    import subprocess

    out_dir.mkdir(parents=True, exist_ok=True)
    pattern = str(out_dir / f"{stream_id}_%03d.mp4")
    total_sec = max(1.0, duration_sec * chunks + 4.0)
    cmd = [
        "ffmpeg",
        "-hide_banner",
        "-loglevel",
...
def copy_segment_dirs_for_inspection(segment_dirs: List[Path], dest_root: Path) -> Optional[Path]:
    """
    在腳本結束清理 /tmp/segmux_* 之前，把各路的 segment 目錄整份複製到 dest_root，
    方便用 ffprobe 檢查每段實際長度與幀數（例如是否約 10 秒、是否只有少數幀）。
    """
    if not segment_dirs:
        return None
    dest_root = dest_root.expanduser().resolve()
    dest_root.mkdir(parents=True, exist_ok=True)
    run_dir = dest_root / f"run_{datetime.now().strftime('%Y%m%d_%H%M%S')}_{os.getpid()}"
    run_dir.mkdir(parents=True, exist_ok=False)
    copied = 0
    for src in segment_dirs:
        if not src.is_dir():
            continue
        sub = run_dir / src.name
        shutil.copytree(src, sub, dirs_exist_ok=True)
        copied += 1
    print(
        f"[SAVE-SEGMENTS] 已複製 {copied} 個錄製目錄至: {run_dir}\n"
        f"  檢查單檔範例: ffprobe -v error -select_streams v:0 "
        f"-show_entries stream=nb_frames,duration,avg_frame_rate -of default=noprint_wrappers=1:nokey=0 "
        f'"{run_dir}/<子目錄>/*.mp4"',
        flush=True,
    )
    return run_dir
...
def wait_file_exists(path: Path, timeout_sec: float) -> None:
    deadline = time.time() + timeout_sec
    while time.time() < deadline:
        if path.exists() and path.stat().st_size > 0:
            return
        time.sleep(0.25)
    raise TimeoutError(f"等待檔案逾時: {path}")


def ensure_video_readable(path: Path, timeout_sec: float = 15.0) -> None:
    import subprocess
    ...
    while time.time() < deadline:
        ...
        if p.returncode == 0 and (p.stdout or "").strip():
            return
        last_err = ((p.stderr or p.stdout or "").strip() or "unknown")[-200:]
        time.sleep(0.25)
    raise TimeoutError(f"影片尚未可讀: {path.name} err={last_err}")
...
def main():
    ...
    parser.add_argument("--streams", type=int, default=25, help="RTSP 路數")
    parser.add_argument("--chunks", type=int, default=180, help="每路總段數")
    ...
    parser.add_argument(
        "--save-segments-dir",
        nargs="?",
        const=str(_BACKEND_SEGMENT_DIR),
        default=None,
        metavar="DIR",
        help=(
            "結束清理前將各 RTSP 錄製目錄（/tmp/segmux_*）整份複製備份。"
            f"只加旗標不帶路徑時預設為 {_BACKEND_SEGMENT_DIR}；"
            "亦可傳自訂目錄（例如其它磁碟路徑）。"
        ),
    )
    ...
    segment_dirs_lock = threading.Lock()
    ...
    def producer(sid: str):
        seg_dir = Path(f"/tmp/segmux_{sid}_{int(time.time()*1000)}")
        with segment_dirs_lock:
            segment_dirs.append(seg_dir)
        proc = None
        try:
            proc = start_rtsp_segmenter(args.rtsp, args.duration, args.chunks, seg_dir, sid)
            for cid in range(1, args.chunks + 1):
                t0 = time.time()
                try:
                    # 只有在「下一段檔名已出現」後，才讀上一段，避免 mp4 尚未寫完（moov atom not found）。
                    # 最後一轮 chunk 也不可先 SIGINT：否則正在寫的上一段會變成極短殘檔（VLM 400 / YOLO 僅 1 幀）。
                    current_idx = cid - 1
                    current_path = seg_dir / f"{sid}_{current_idx:03d}.mp4"
                    next_path = seg_dir / f"{sid}_{cid:03d}.mp4"
                    wait_file_exists(next_path, timeout_sec=max(20.0, args.duration + 12.0))

                    wait_file_exists(current_path, timeout_sec=max(20.0, args.duration + 12.0))
                    ensure_video_readable(current_path, timeout_sec=12.0)
                    path = current_path
    ...
    # 所有批次都完成後才清理 segment 暫存，避免 batch 尚在讀檔時被提前刪除。
    with segment_dirs_lock:
        dirs_to_cleanup = list(segment_dirs)
    save_dir = getattr(args, "save_segments_dir", None)
    if save_dir and str(save_dir).strip():
        try:
            copy_segment_dirs_for_inspection(dirs_to_cleanup, Path(save_dir))
        except Exception as e:
            print(f"[SAVE-SEGMENTS] 複製失敗（仍會清理暫存）: {e}", flush=True)
